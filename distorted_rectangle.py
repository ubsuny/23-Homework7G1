...
delta_y = 0.05
delta_z = 0.08
# Define distorted-rectangle configuration for Na+ and Cl- ions
# Distortion in the x, y, z-direction for the third Na+ ion
r_na = np.array([[0, 0, 0], [a, 0, a], [0+delta_x, b + delta_y, a+delta_z], [a, b, 0]
])
...
ax.set_ylabel('b (nm)')
ax.set_zlabel('h (nm)')
ax.text2D(0.05, 0.95, "Optimized potential for distorted-rectangle configuration: -28.18 eV", transform=ax.transAxes, color='blue', weight='bold')
...
plt.title('Distorted-Rectangle: Potential Energy vs. Optimization Steps (Using BFGS)', fontsize=10, fontweight='bold')
plt.show()
